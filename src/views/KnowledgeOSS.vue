      ...
      <div class="dialog">
        ...
        <div class="dialog-content">
          <div class="upload-area" @click="triggerFileInput" @drop="handleDrop" @dragover.prevent>
            ...
            <p>点击或拖拽上传文档</p>
            <p class="upload-hint">支持 .txt、.md、.pdf、.doc、.docx 等格式，支持大文件直传</p>
          </div>
    ...
    <div v-if="viewingDocument" class="dialog-overlay" @click.self="closeViewDialog">
      <div class="dialog view-dialog">
        ...
        <div class="dialog-content">
          ...
          <!-- 如果是OSS文件，显示下载按钮 -->
          <div v-if="viewingDocument.oss_key" class="document-actions">
            <button @click="downloadDocument(viewingDocument)" class="download-btn">
              📥 下载原始文件
            </button>
          </div>
        </div>
...
import { uploadDocumentToOSS, uploadMultipleDocuments, getDocumentUrl } from '../utils/ossDocumentUpload.js'
  ...
  try {
    ...
    const results = await uploadMultipleDocuments(
      ...
      (progress, current, total) => {
        ...
        currentFileIndex.value = current
    ...
    if (succeeded > 0) {
      ...
      window.$toast?.error('所有文件上传失败，请重试')
    }
...
async function downloadDocument(doc) {
  if (!doc.oss_key || !doc.original_name) {
    window.$toast?.error('文档信息不完整')
    return
  }

  try {
    const success = await downloadFile(doc.oss_key, doc.original_name)
    if (!success) {
      window.$toast?.error('下载失败')
    } else {
      window.$toast?.success('开始下载')
    }
  } catch (error) {
